...
public class ArticleBachTest {
  ...
  void execute_shouldCallCollectorsAndIncrementCounters_andExitCompleted() throws Exception {
    ...
    when(naverNewsCollector.naverArticleCollect()).thenReturn(newNaverArticleCounts);
    when(choSunCollector.chousunArticleCollect()).thenReturn(newChoSunArticleCounts);
    when(hanKyungCollector.hankyungArticleCollect()).thenReturn(newHanKyungArticleCounts);
